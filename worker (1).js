...
/* ---- Xero accounting helpers (kpi-spec.md: Revenue/Income, Cost of Sales,
   wages+super within Operating Expenses, Overheads = OpEx - wages/super) ---- */
async function xeroTenant(h) {
  ...
  if (!c) { const e = new Error('no Xero organisation connected'); e.status = 401; throw e; }
...
function xeroFindSection(rows, titleTest) {
  for (const r of rows || []) {
    ...
    if (r.Rows) { const found = xeroFindSection(r.Rows, titleTest); if (found) return found; }
  }
  return null;
}
function xeroSectionTotal(section, colCount) {
  const totals = new Array(colCount).fill(0);
  const summary = (section.Rows || []).find(r => r.RowType === 'SummaryRow');
  if (summary) {
    for (let i = 0; i < colCount; i++) totals[i] = xeroNum(summary.Cells[i + 1] && summary.Cells[i + 1].Value);
    return totals;
  }
  for (const r of section.Rows || []) {
    if (r.RowType === 'Row') {
      for (let i = 0; i < colCount; i++) totals[i] += xeroNum(r.Cells[i + 1] && r.Cells[i + 1].Value);
    }
  }
  return totals;
}
/* Proposes wage/super lines by keyword; kpi-spec.md requires confirming the
   exact list with the owner during reconciliation (a business question). */
const XERO_WAGE_KEYWORDS = /wages|salaries|superannuation|super|payroll|annual leave|long service|workcover/i;
function xeroWagesSuperTotals(opexSection, colCount) {
  const totals = new Array(colCount).fill(0);
  for (const r of (opexSection && opexSection.Rows) || []) {
    if (r.RowType === 'Row' && XERO_WAGE_KEYWORDS.test((r.Cells[0] && r.Cells[0].Value) || '')) {
      for (let i = 0; i < colCount; i++) totals[i] += xeroNum(r.Cells[i + 1] && r.Cells[i + 1].Value);
    }
  }
  return totals;
}
function xeroParsePL(json, colCount) {
  const report = json && json.Reports && json.Reports[0];
  const rows = (report && report.Rows) || [];
  const incomeSection = xeroFindSection(rows, t => /^income$/i.test(t.trim()) || (/revenue|trading income/i.test(t) && !/other income/i.test(t)));
  const cogsSection = xeroFindSection(rows, t => /cost of sales/i.test(t));
  const opexSection = xeroFindSection(rows, t => /operating expenses/i.test(t));
  const revenue = incomeSection ? xeroSectionTotal(incomeSection, colCount) : new Array(colCount).fill(null);
  const cogs = cogsSection ? xeroSectionTotal(cogsSection, colCount) : new Array(colCount).fill(null);
  const opexTotal = opexSection ? xeroSectionTotal(opexSection, colCount) : new Array(colCount).fill(null);
  const wagesSuper = opexSection ? xeroWagesSuperTotals(opexSection, colCount) : new Array(colCount).fill(null);
  const overheads = opexTotal.map((v, i) => (v == null || wagesSuper[i] == null) ? null : v - wagesSuper[i]);
  ...
  */
  accounting: {
    ...
    async status(env, h) {
      ...
      return { connected: true, org: t.tenantName, sandbox: /demo company/i.test(t.tenantName || '') };
    },
    async fetchRange(env, h, q) {
      const t = await xeroTenant(h);
      const json = await h.fetchJson(
        'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?fromDate=' + q.from + '&toDate=' + q.to,
        { headers: { 'Xero-Tenant-Id': t.tenantId, 'Accept': 'application/json' } }
      );
      const p = xeroParsePL(json, 1);
    ...
    async fetchMonthly(env, h, q) {
      ...
      for (let i = 0; i < months.length; i += 12) {
        ...
        const json = await h.fetchJson(
          'https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss?fromDate=' + fromDate +
          '&periods=' + chunk.length + '&timeframe=MONTH',
          { headers: { 'Xero-Tenant-Id': t.tenantId, 'Accept': 'application/json' } }
        );
        const p = xeroParsePL(json, chunk.length);
        revenue.push(...p.revenue); cogs.push(...p.cogs); wagesSuper.push(...p.wagesSuper); overheads.push(...p.overheads);
      }
